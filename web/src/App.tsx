...
export default function App() {
  ...
  const [auctionAbi, setAuctionAbi] = useState<any[]>([])
  const [erc20Abi, setErc20Abi] = useState<any[]>([])

  useEffect(() => { loadAuctions(); loadAbis() }, [])

  async function loadAbis() {
    try {
      const a1 = await (await fetch(`${apiUrl}/abi/auction`)).json()
      const a2 = await (await fetch(`${apiUrl}/abi/erc20`)).json()
      setAuctionAbi(a1.abi || [])
      setErc20Abi(a2.abi || [])
    } catch { }
  }
  ...
  async function bidWithETH(auctionAddress: string) {
    ...
    const provider = new ethers.BrowserProvider(ethereum)
    const signer = await provider.getSigner()
    if (!auctionAbi.length) { alert('ABI 未加载'); return }
    const inst = new ethers.Contract(auctionAddress, auctionAbi, signer)
    try {
  ...
  async function bidWithERC20(auctionAddress: string) {
    ...
    const provider = new ethers.BrowserProvider(ethereum)
    const signer = await provider.getSigner()
    if (!auctionAbi.length || !erc20Abi.length) { alert('ABI 未加载'); return }
    const inst = new ethers.Contract(auctionAddress, auctionAbi, signer)
