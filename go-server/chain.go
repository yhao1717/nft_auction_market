...
const aggregatorABIJSON = `[{"inputs":[],"name":"latestRoundData","outputs":[{"internalType":"uint80","name":"roundId","type":"uint80"},{"internalType":"int256","name":"answer","type":"int256"},{"internalType":"uint256","name":"startedAt","type":"uint256"},{"internalType":"uint256","name":"updatedAt","type":"uint256"},{"internalType":"uint80","name":"answeredInRound","type":"uint80"}],"stateMutability":"view","type":"function"}]`

const factoryABIJSON = `[{"inputs":[],"name":"ethUsdFeed","outputs":[{"internalType":"contract AggregatorV3Interface","name":"","type":"address"}],"stateMutability":"view","type":"function"}]`
...
const auctionABIJSON = `[
  ...
  {"inputs":[],"name":"highestBidder","outputs":[{"type":"address"}],"stateMutability":"view","type":"function"},
  {"inputs":[],"name":"highestCurrency","outputs":[{"type":"address"}],"stateMutability":"view","type":"function"},
  {"inputs":[],"name":"highestAmount","outputs":[{"type":"uint256"}],"stateMutability":"view","type":"function"},
  {"inputs":[],"name":"highestUsd","outputs":[{"type":"uint256"}],"stateMutability":"view","type":"function"},
  {"inputs":[],"name":"settled","outputs":[{"type":"bool"}],"stateMutability":"view","type":"function"}
]`
...
func NewChain(rpcURL, factoryAddr string) (*Chain, error) {
    ...
    agABI, err := abi.JSON(strings.NewReader(aggregatorABIJSON))
    if err != nil { return nil, err }
    fABI, err := abi.JSON(strings.NewReader(factoryABIJSON))
    if err != nil { return nil, err }
    aABI, err := abi.JSON(strings.NewReader(auctionABIJSON))
    if err != nil { return nil, err }
...
func (c *Chain) ReadAuction(ctx context.Context, addr string) (*AuctionState, error) {
    ...
    read := func(name string) ([]byte, error) { return c.cli.CallContract(ctx, ethereum.CallMsg{To: &a, Data: c.auctionABI.Pack(name)}, nil) }
